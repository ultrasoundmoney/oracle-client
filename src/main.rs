...
async fn run_oracle_node(
    ...
    mut slot_stream: impl Stream<Item = u64> + std::marker::Unpin,
) -> Result<()> {
    while let Some(slot_number) = slot_stream.next().await {
...
async fn get_slot_stream<'a>(provider: &'a Provider<Ws>) -> Result<impl Stream<Item = u64> + 'a> {
    ...
    let slot_stream = block_stream.map(|block| block.number.unwrap().as_u64());
    Ok(slot_stream)
}
...
async fn main() -> Result<()> {
    ...
    // TODO: Replace with stream that returns the current slot instead of mined blocknumbers
    let provider =
        Provider::<Ws>::connect("wss://mainnet.infura.io/ws/v3/c60b0bb42f8a4c6481ecd229eddaca27")
            .await?;

    let slot_stream = get_slot_stream(&provider).await?;
